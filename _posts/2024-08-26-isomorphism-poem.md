...
**Proof**: Define $$\theta$$ on the cosets of $$K$$ in $$A$$ such that $$\theta(gK) = \phi(g)$$. This is well-defined, since if $$gK = hK$$ (as sets), then $$h^{-1}gK = K$$, so $$h^{-1}g \in K$$. Therefore, we know that $$\begin{align*} \phi(h^{-1}g) &= e \\ \phi(h) &= \phi(g)\end{align*}$$, so in particular $$\theta(hK) = \theta(gK)$$ when $$gK = hK$$. 
...
Then also we need to show that $$\theta$$ is injective. If $$\theta(gK) = \theta(hK)$$, then $$\phi(g) = \phi(h)$$, so $$h^{-1} \cdot g \in K$$. So $$\begin{align*} hK &= h \cdot h^{-1}gK\\ &= gK$$. So $$\theta(hK)=\theta(gK)$$ implies $$hK = gK$$, and $$\theta$$ is injective.
